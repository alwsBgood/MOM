$("#page-preloader").delay(200).fadeOut();

if (localStorage.name && localStorage.email && localStorage.phone)  {
  // запись сохраненных данных сразу в поля, если надо
  $('input[name="entry.1750371417"]').val(localStorage.name);
  $('input[type="entry.1382825857"]').val(localStorage.email);
  $('input[type="entry.522199749"]').val(localStorage.phone);
}
//Форма отправки 2.0
$(function() {
  $("[name=send]").click(function () {
    ...
    localStorage.name = form.find('input[name="entry.1750371417"]').val();
    localStorage.email = form.find('input[type="entry.1382825857"]').val();
    localStorage.phone = form.find('input[type="entry.522199749"]').val();
